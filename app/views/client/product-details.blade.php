@extends('client.layout.main')
@section('content')

    <!-- ====== Breadcrumbs Section Start -->
    <section class="ef nf">
      <div class="a da">
        <div class="oc">
          <div class="ug vg ul qm rm">
            <ul class="sb je le">
              <li class="sb je">
                <a
                  href="index.html"
                  class="sb je ui bj gj ik"
                >
                  <span class="uh">
                    <svg
                      width="15"
                      height="15"
                      viewBox="0 0 15 15"
                      class="kg"
                    >
                      <path
                        d="M13.3503 14.6503H10.2162C9.51976 14.6503 8.93937 14.0697 8.93937 13.3729V10.8182C8.93937 10.5627 8.73043 10.3537 8.47505 10.3537H6.54816C6.29279 10.3537 6.08385 10.5627 6.08385 10.8182V13.3497C6.08385 14.0464 5.50346 14.627 4.80699 14.627H1.62646C0.929989 14.627 0.349599 14.0464 0.349599 13.3497V5.24431C0.349599 4.89594 0.535324 4.5708 0.837127 4.385L6.96604 0.506501C7.29106 0.297479 7.73216 0.297479 8.05717 0.506501L14.1861 4.385C14.4879 4.5708 14.6504 4.89594 14.6504 5.24431V13.3265C14.6504 14.0697 14.07 14.6503 13.3503 14.6503ZM6.52495 9.54086H8.45184C9.14831 9.54086 9.7287 10.1215 9.7287 10.8182V13.3497C9.7287 13.6052 9.93764 13.8142 10.193 13.8142H13.3503C13.6057 13.8142 13.8146 13.6052 13.8146 13.3497V5.26754C13.8146 5.19786 13.7682 5.12819 13.7218 5.08174L7.61608 1.20324C7.54643 1.15679 7.45357 1.15679 7.40714 1.20324L1.27822 5.08174C1.20858 5.12819 1.18536 5.19786 1.18536 5.26754V13.3729C1.18536 13.6284 1.3943 13.8374 1.64967 13.8374H4.80699C5.06236 13.8374 5.2713 13.6284 5.2713 13.3729V10.8182C5.24809 10.1215 5.82848 9.54086 6.52495 9.54086Z"
                      />
                      <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M7.51145 1.55106L13.465 5.33294V13.3497C13.465 13.412 13.4126 13.4644 13.3503 13.4644H10.193C10.1307 13.4644 10.0783 13.412 10.0783 13.3497V10.8182C10.0783 9.92832 9.34138 9.19112 8.45184 9.19112H6.52495C5.63986 9.19112 4.89529 9.92522 4.9217 10.8237V13.3729C4.9217 13.4352 4.86929 13.4877 4.80699 13.4877H1.64967C1.58738 13.4877 1.53496 13.4352 1.53496 13.3729V5.33311L7.51145 1.55106ZM1.27822 5.08174L7.40714 1.20324C7.45357 1.15679 7.54643 1.15679 7.61608 1.20324L13.7218 5.08174C13.7682 5.12819 13.8146 5.19786 13.8146 5.26754V13.3497C13.8146 13.6052 13.6057 13.8142 13.3503 13.8142H10.193C9.93764 13.8142 9.7287 13.6052 9.7287 13.3497V10.8182C9.7287 10.1215 9.14831 9.54086 8.45184 9.54086H6.52495C5.82848 9.54086 5.24809 10.1215 5.2713 10.8182V13.3729C5.2713 13.6284 5.06236 13.8374 4.80699 13.8374H1.64967C1.3943 13.8374 1.18536 13.6284 1.18536 13.3729V5.26754C1.18536 5.19786 1.20858 5.12819 1.27822 5.08174ZM13.3503 15H10.2162C9.32668 15 8.58977 14.2628 8.58977 13.3729V10.8182C8.58977 10.7559 8.53735 10.7035 8.47505 10.7035H6.54816C6.48587 10.7035 6.43345 10.7559 6.43345 10.8182V13.3497C6.43345 14.2396 5.69654 14.9768 4.80699 14.9768H1.62646C0.736911 14.9768 0 14.2396 0 13.3497V5.24431C0 4.77131 0.251303 4.33591 0.651944 4.08836L6.77814 0.211575C7.21781 -0.0705255 7.80541 -0.0705251 8.24508 0.211576C8.24546 0.211821 8.24584 0.212066 8.24622 0.212311L14.3713 4.08838C14.7853 4.34424 15 4.78759 15 5.24431V13.3265C15 14.2587 14.2671 15 13.3503 15ZM14.1861 4.385L8.05717 0.506501C7.73216 0.297479 7.29106 0.297479 6.96604 0.506501L0.837127 4.385C0.535324 4.5708 0.349599 4.89594 0.349599 5.24431V13.3497C0.349599 14.0464 0.929989 14.627 1.62646 14.627H4.80699C5.50346 14.627 6.08385 14.0464 6.08385 13.3497V10.8182C6.08385 10.5627 6.29279 10.3537 6.54816 10.3537H8.47505C8.73043 10.3537 8.93937 10.5627 8.93937 10.8182V13.3729C8.93937 14.0697 9.51976 14.6503 10.2162 14.6503H13.3503C14.07 14.6503 14.6504 14.0697 14.6504 13.3265V5.24431C14.6504 4.89594 14.4879 4.5708 14.1861 4.385Z"
                      />
                    </svg>
                  </span>
                  Home
                </a>
                <span class="wg hj">
                  <svg
                    width="7"
                    height="12"
                    viewBox="0 0 7 12"
                    class="kg"
                  >
                    <path
                      d="M0.879233 11.4351C0.808625 11.4351 0.720364 11.3998 0.667408 11.3469C0.543844 11.2233 0.543844 11.0291 0.649756 10.9056L5.09807 6.17483C5.18633 6.08657 5.18633 5.92771 5.09807 5.82179L0.649756 1.09105C0.526192 0.967487 0.543844 0.773315 0.667408 0.649751C0.790972 0.526187 0.985145 0.543839 1.10871 0.667403L5.55702 5.39815C5.85711 5.73353 5.85711 6.26309 5.55702 6.58083L1.10871 11.3292C1.0381 11.3998 0.967493 11.4351 0.879233 11.4351Z"
                    />
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
                      d="M0.229332 10.5281L4.48868 5.99831L0.24288 1.48294C-0.133119 1.09849 -0.0312785 0.549591 0.267983 0.25033C0.652758 -0.134445 1.2069 -0.0332381 1.50812 0.267982L1.52041 0.280272L5.9781 5.02138C6.46442 5.56491 6.47872 6.42661 5.96853 6.96778V6.96778L1.50834 11.7289C1.36051 11.8767 1.15353 12 0.879227 12C0.660517 12 0.428074 11.9064 0.267983 11.7463C-0.0719543 11.4064 -0.0699959 10.8773 0.220873 10.538L0.229332 10.5281ZM5.55702 6.58083C5.85711 6.26309 5.85711 5.73353 5.55702 5.39815L1.10871 0.667403C0.985145 0.543839 0.790972 0.526187 0.667408 0.649751C0.543844 0.773315 0.526192 0.967487 0.649756 1.09105L5.09807 5.82179C5.18633 5.92771 5.18633 6.08657 5.09807 6.17483L0.649756 10.9056C0.543844 11.0291 0.543844 11.2233 0.667408 11.3469C0.720364 11.3998 0.808625 11.4351 0.879233 11.4351C0.967493 11.4351 1.0381 11.3998 1.10871 11.3292L5.55702 6.58083Z"
                    />
                  </svg>
                </span>
              </li>

              <li class="ui bj ij">
                Product Details
              </li>
            </ul>
          </div>
        </div>
      </div>
    </section>
    <!-- ====== Breadcrumbs Section End -->

    <!-- ====== Products Details Start -->
    <section
      x-data="
        {
          productOne: true,
          productTwo: false,
          productThree: false,
          productFour: false,
          modalOpen: false
        }
      "
      class="wf vh"
    >
      <div class="a da">
        <div
          class="se ze af nf wf sg"
        >
          <div class="ea sb he">
            <div class="oc vg ao/2">
              <div class="la in mn sp">
                <div class="e ma se">
                  <button
                    @click=" modalOpen = true "
                    class="d o p sb gc xc je le ye wf gj yj"
                  >
                    <svg
                      class="yb qc il ml"
                      viewBox="0 0 30 30"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M28.8281 24.4219L22.0313 17.5781C21.3281 16.875 20.0625 16.875 19.3594 17.5781L19.0312 17.9062L17.3438 16.1719C20.4844 12.4219 20.2969 6.84375 16.7813 3.375C15 1.59375 12.6094 0.609375 10.0781 0.609375C7.54688 0.609375 5.15625 1.59375 3.375 3.375C-0.328125 7.07812 -0.328125 13.125 3.375 16.7812C5.15625 18.5625 7.54688 19.5469 10.0781 19.5469C12.3281 19.5469 14.4844 18.75 16.1719 17.2969L17.9063 19.0312L17.5781 19.3594C16.8281 20.1094 16.8281 21.2812 17.5781 22.0312L24.375 28.8281C24.75 29.2031 25.2188 29.3906 25.6875 29.3906C26.2031 29.3906 26.6719 29.2031 27 28.8281L28.7813 27.0469C29.5781 26.3437 29.5781 25.1719 28.8281 24.4219ZM10.0781 17.9531C7.96875 17.9531 6 17.1562 4.54688 15.6562C1.5 12.6094 1.5 7.59375 4.54688 4.54687C6.04688 3.04687 8.01563 2.25 10.0781 2.25C12.1875 2.25 14.1563 3.04687 15.6094 4.54687C18.6563 7.59375 18.6563 12.6094 15.6094 15.6562C14.1563 17.1094 12.1875 17.9531 10.0781 17.9531ZM27.7031 25.9219L25.9219 27.7031C25.8281 27.7969 25.6875 27.7969 25.5938 27.7031L18.75 20.9062C18.6563 20.8125 18.6563 20.6719 18.75 20.5781L20.5313 18.7969C20.5781 18.75 20.6719 18.75 20.7188 18.75C20.7656 18.75 20.8125 18.75 20.9063 18.7969L27.7031 25.5937C27.7969 25.6875 27.7969 25.8281 27.7031 25.9219Z"
                        fill="currentColor"
                      />
                      <path
                        d="M12.5625 9.28125H10.9219V7.64062C10.9219 7.17187 10.5469 6.79688 10.0781 6.79688C9.60938 6.79688 9.23438 7.17187 9.23438 7.64062V9.28125H7.59375C7.125 9.28125 6.75 9.65625 6.75 10.125C6.75 10.5938 7.125 10.9687 7.59375 10.9687H9.23438V12.6094C9.23438 13.0781 9.60938 13.4531 10.0781 13.4531C10.5469 13.4531 10.9219 13.0781 10.9219 12.6094V10.9687H12.5625C13.0313 10.9687 13.4063 10.5938 13.4063 10.125C13.4063 9.65625 13.0313 9.28125 12.5625 9.28125Z"
                        fill="currentColor"
                      />
                    </svg>
                  </button>
                  <img
                    src="src/assets/ecom-images/products-details/details-02/big-image-01.jpg"
                    alt="products-details"
                    class="oc ve"
                    x-show="productOne "
                  />
                  <img
                    src="src/assets/ecom-images/products-details/details-02/big-image-02.jpg"
                    alt="products-details"
                    class="oc ve"
                    x-show="productTwo "
                  />
                  <img
                    src="src/assets/ecom-images/products-details/details-02/big-image-03.jpg"
                    alt="products-details"
                    class="oc ve"
                    x-show="productThree "
                  />
                  <img
                    src="src/assets/ecom-images/products-details/details-02/big-image-04.jpg"
                    alt="products-details"
                    class="oc ve"
                    x-show="productFour "
                  />
                </div>
                <div
                  class="ia sb je me"
                >
                  <div class="pc/4 lh">
                    <button
                      @click="productOne = true; productTwo = false; productThree = false; productFour = false "
                      :class=" productOne === true ? 'rf' : 'pf' "
                      class="oc se ve df"
                    >
                      <img
                        src="src/assets/ecom-images/products-details/details-02/thumbnail-01.jpg"
                        alt="thumbnail-01"
                        class="oc"
                      />
                    </button>
                  </div>
                  <div class="pc/4 lh">
                    <button
                      @click="productTwo = true; productOne = false; productThree = false; productFour = false "
                      :class=" productTwo === true ? 'rf' : 'pf' "
                      class="oc se ve df"
                    >
                      <img
                        src="src/assets/ecom-images/products-details/details-02/thumbnail-02.jpg"
                        alt="thumbnail-01"
                        class="oc"
                      />
                    </button>
                  </div>
                  <div class="pc/4 lh">
                    <button
                      @click="productThree = true; productOne = false; productTwo = false; productFour = false "
                      :class=" productThree === true ? 'rf' : 'pf' "
                      class="oc se ve df"
                    >
                      <img
                        src="src/assets/ecom-images/products-details/details-02/thumbnail-03.jpg"
                        alt="thumbnail-01"
                        class="oc"
                      />
                    </button>
                  </div>
                  <div class="pc/4 lh">
                    <button
                      @click="productFour = true; productOne = false; productTwo = false; productThree = false "
                      :class=" productFour === true ? 'rf' : 'pf' "
                      class="oc se ve df"
                    >
                      <img
                        src="src/assets/ecom-images/products-details/details-02/thumbnail-04.jpg"
                        alt="thumbnail-01"
                        class="oc"
                      />
                    </button>
                  </div>
                </div>
              </div>
            </div>

            <div class="oc vg ao/2">
              <div>
                <h2
                  class="oa vi bj gj yl lp oq"
                >
                  Macbook Pro M1 8/256GB
                </h2>

                <p
                  class="oa ui dj hj"
                >
                  Lorem ipsum dolor sit amet, consectetur adipiscing elit. Morbi
                  non erat quam. Vestibulum aliquam nibh.
                </p>

                <div class="sb he je">
                  <div class="sb je ji">
                    <h3
                      class="wi bj gj bm _m lp oq"
                    >
                      <span class="hi"> $1200.99 </span>
                      <span class="hj qj">
                        $1500.99
                      </span>
                    </h3>
                  </div>
                  <div class="sb je">
                    <span class="uh">
                      <svg
                        width="20"
                        height="20"
                        viewBox="0 0 20 20"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M10 19.625C4.6875 19.625 0.40625 15.3125 0.40625 10C0.40625 4.6875 4.6875 0.40625 10 0.40625C15.3125 0.40625 19.625 4.6875 19.625 10C19.625 15.3125 15.3125 19.625 10 19.625ZM10 1.5C5.3125 1.5 1.5 5.3125 1.5 10C1.5 14.6875 5.3125 18.5313 10 18.5313C14.6875 18.5313 18.5313 14.6875 18.5313 10C18.5313 5.3125 14.6875 1.5 10 1.5Z"
                          fill="#13C296"
                        />
                        <path
                          d="M8.9375 12.1875C8.71875 12.1875 8.53125 12.125 8.34375 11.9687L6.28125 9.96875C6.0625 9.75 6.0625 9.40625 6.28125 9.1875C6.5 8.96875 6.84375 8.96875 7.0625 9.1875L8.9375 11.0312L12.9375 7.15625C13.1562 6.9375 13.5 6.9375 13.7188 7.15625C13.9375 7.375 13.9375 7.71875 13.7188 7.9375L9.5625 12C9.34375 12.125 9.125 12.1875 8.9375 12.1875Z"
                          fill="#13C296"
                        />
                      </svg>
                    </span>
                    <span class="ui dj gj">
                      In Stock
                    </span>
                  </div>
                </div>

                <div class="jb">
                  <h4 class="xa wi bj gj">
                    Color
                  </h4>
                  <div class="sb je pe">
                    <div class="e">
                      <input
                        type="radio"
                        name="productColor"
                        id="black"
                        class="productColor b"
                      />
                      <label
                        for="black"
                        class="sb hc yc ce je le ye df sf"
                      >
                        <span
                          class="hc yc ye bg"
                        ></span>
                      </label>
                    </div>
                    <div class="e">
                      <input
                        type="radio"
                        name="productColor"
                        id="black2"
                        class="productColor b"
                      />
                      <label
                        for="black2"
                        class="sb hc yc ce je le ye df qf"
                      >
                        <span
                          class="hc yc ye cg"
                        ></span>
                      </label>
                    </div>
                    <div class="e">
                      <input
                        type="radio"
                        name="productColor"
                        id="black3"
                        class="productColor b"
                      />
                      <label
                        for="black3"
                        class="sb hc yc ce je le ye df nf"
                      >
                        <span
                          class="hc yc ye dg"
                        ></span>
                      </label>
                    </div>
                  </div>
                </div>

                <div class="jb">
                  <h4 class="xa wi bj gj">
                    Specification
                  </h4>
                  <div class="qe">
                    <p
                      class="ui dj hj dl"
                    >
                      <span class="qb zc"> Memory : </span>
                      <span> 8GB unified memory </span>
                    </p>
                    <p
                      class="ui dj hj dl"
                    >
                      <span class="qb zc"> Storage : </span>
                      <span> 256GB/512GB SSD storage </span>
                    </p>
                    <p
                      class="ui dj hj dl"
                    >
                      <span class="qb zc"> Display : </span>
                      <span> 13-inch Retina display with True Tone </span>
                    </p>
                    <p
                      class="ui dj hj dl"
                    >
                      <span class="qb zc"> Processor : </span>
                      <span> Apple M1 chip with 8-core CPU & GPU </span>
                    </p>
                  </div>
                </div>

                <div class="jb sb he">
                  <div class="eb oa">
                    <h4
                      class="xa wi bj gj"
                    >
                      RAM
                    </h4>
                    <div class="sb je oe">
                      <div>
                        <input
                          type="radio"
                          name="ram"
                          id="8gb"
                          class="ram-size b"
                        />
                        <label
                          for="8gb"
                          class="rb ce xe af nf mh vg ui bj gj"
                        >
                          8GB
                        </label>
                      </div>
                      <div>
                        <input
                          type="radio"
                          name="ram"
                          id="16gb"
                          class="ram-size b"
                        />
                        <label
                          for="16gb"
                          class="rb ce xe af nf mh vg ui bj gj"
                        >
                          16GB
                        </label>
                      </div>
                    </div>
                  </div>
                  <div class="oa">
                    <h4
                      class="xa wi bj gj"
                    >
                      Storage
                    </h4>
                    <div class="sb je oe">
                      <div>
                        <input
                          type="radio"
                          name="storage"
                          id="256gb"
                          class="ram-size b"
                        />
                        <label
                          for="256gb"
                          class="rb ce xe af nf mh vg ui bj gj"
                        >
                          256GB
                        </label>
                      </div>
                      <div>
                        <input
                          type="radio"
                          name="storage"
                          id="512gb"
                          class="ram-size b"
                        />
                        <label
                          for="512gb"
                          class="rb ce xe af nf mh vg ui bj gj"
                        >
                          512GB
                        </label>
                      </div>
                      <div>
                        <input
                          type="radio"
                          name="storage"
                          id="1tb"
                          class="ram-size b"
                        />
                        <label
                          for="1tb"
                          class="rb ce xe af nf mh vg ui bj gj"
                        >
                          1TB
                        </label>
                      </div>
                    </div>
                  </div>
                </div>

                <div class="sb he">
                  <div
                    x-data="
                      {
                        quantity: 1
                      }
                    "
                    class="ya xa tb je xe af nf ui dj gj"
                  >
                    <span
                      @click=" quantity > 1 && quantity-- "
                      class="rb ce de nh vg"
                    >
                      -
                    </span>
                    <span
                      class="rb nh vg"
                      x-text="quantity"
                    >
                    </span>
                    <span
                      @click=" quantity++ "
                      class="rb ce de nh vg"
                    >
                      +
                    </span>
                  </div>

                  <div class="ya xa">
                    <button
                      class="tb je le we yf mh yg si ui bj jj hk sg qg"
                    >
                    <span>
          <svg width="20" height="20" viewBox="0 0 20 20" class="hk ya">
            <path d="M18.0738 0.849609H16.0676C15.8171 0.850867 15.5754 0.942155 15.3867 1.10681C15.1979 1.27146 15.0747 1.4985 15.0394 1.74648L14.4769 5.80898H13.3957V3.40273C13.3933 3.16503 13.2982 2.93766 13.1307 2.76899C12.9632 2.60031 12.7365 2.50367 12.4988 2.49961H10.0801C10.0533 2.27727 9.94785 2.07185 9.78277 1.92052C9.61769 1.7692 9.40389 1.68196 9.18007 1.67461H6.45194C6.3269 1.67779 6.20376 1.70595 6.08977 1.75745C5.97578 1.80895 5.87325 1.88274 5.78821 1.97447C5.70318 2.0662 5.63736 2.17402 5.59464 2.29158C5.55191 2.40914 5.53315 2.53406 5.53944 2.65898V3.32461H3.14882C2.91201 3.32126 2.68348 3.41171 2.51312 3.57622C2.34275 3.74073 2.24437 3.96595 2.23944 4.20273V5.81211C2.09795 5.81502 1.95882 5.84901 1.83192 5.91165C1.70501 5.97429 1.59343 6.06406 1.50507 6.17461C1.41421 6.29057 1.35071 6.42556 1.31933 6.5695C1.28794 6.71343 1.28948 6.8626 1.32382 7.00586C1.32248 7.02146 1.32248 7.03714 1.32382 7.05273L3.23632 12.9371C3.29128 13.1429 3.41335 13.3244 3.58318 13.4529C3.753 13.5814 3.96087 13.6495 4.17382 13.6465H13.3863C13.7359 13.6461 14.0736 13.5193 14.3372 13.2896C14.6008 13.0599 14.7725 12.7428 14.8207 12.3965L16.2488 2.09961H18.0707C18.2365 2.09961 18.3954 2.03376 18.5126 1.91655C18.6298 1.79934 18.6957 1.64037 18.6957 1.47461C18.6957 1.30885 18.6298 1.14988 18.5126 1.03267C18.3954 0.915457 18.2365 0.849609 18.0707 0.849609H18.0738ZM12.1363 5.79336H10.0926V3.74961H12.1457L12.1363 5.79336ZM6.78944 2.91523H8.84257V3.39961V5.79023H6.78944V2.91523ZM3.48944 4.56523H5.53944V5.79336H3.48944V4.56523ZM13.5832 12.2246C13.5771 12.2724 13.5537 12.3163 13.5174 12.3479C13.4811 12.3796 13.4345 12.3969 13.3863 12.3965H4.37382L2.65194 7.04961H14.3019L13.5832 12.2246Z"></path>
            <path d="M5.31172 15.1133C4.9118 15.1102 4.51997 15.226 4.18594 15.4459C3.85191 15.6659 3.59073 15.9801 3.43553 16.3487C3.28034 16.7173 3.23813 17.1236 3.31425 17.5163C3.39037 17.9089 3.58139 18.2701 3.86309 18.5539C4.14478 18.8378 4.50445 19.0317 4.89647 19.1108C5.28849 19.19 5.6952 19.1509 6.06499 18.9986C6.43477 18.8463 6.75099 18.5876 6.97351 18.2553C7.19603 17.9229 7.31483 17.532 7.31485 17.1321C7.31608 16.868 7.26522 16.6062 7.16518 16.3617C7.06515 16.1172 6.91789 15.8949 6.73184 15.7074C6.5458 15.5199 6.3246 15.3709 6.08092 15.269C5.83725 15.167 5.57586 15.1142 5.31172 15.1133ZM5.31172 17.9008C5.15905 17.9039 5.00891 17.8615 4.88045 17.7789C4.75199 17.6964 4.65103 17.5774 4.59045 17.4373C4.52986 17.2971 4.51239 17.1421 4.54026 16.9919C4.56814 16.8418 4.64009 16.7034 4.74695 16.5943C4.85382 16.4852 4.99075 16.4104 5.14028 16.3795C5.28981 16.3485 5.44518 16.3628 5.58656 16.4205C5.72794 16.4782 5.84894 16.5767 5.93412 16.7034C6.0193 16.8302 6.06481 16.9794 6.06484 17.1321C6.06651 17.3338 5.9882 17.5279 5.84705 17.672C5.70589 17.8161 5.51341 17.8984 5.31172 17.9008Z"></path>
            <path d="M12.9484 15.1133C12.5485 15.1102 12.1567 15.226 11.8227 15.4459C11.4886 15.6659 11.2274 15.9801 11.0723 16.3487C10.9171 16.7173 10.8748 17.1236 10.951 17.5163C11.0271 17.9089 11.2181 18.2701 11.4998 18.5539C11.7815 18.8378 12.1412 19.0317 12.5332 19.1108C12.9252 19.19 13.3319 19.1509 13.7017 18.9986C14.0715 18.8463 14.3877 18.5876 14.6102 18.2553C14.8327 17.9229 14.9516 17.532 14.9516 17.1321C14.9532 16.5989 14.7432 16.0868 14.3676 15.7083C13.9921 15.3298 13.4816 15.1158 12.9484 15.1133ZM12.9484 17.9008C12.7958 17.9039 12.6456 17.8615 12.5172 17.7789C12.3887 17.6964 12.2878 17.5774 12.2272 17.4373C12.1666 17.2971 12.1491 17.1421 12.177 16.9919C12.2049 16.8418 12.2768 16.7034 12.3837 16.5943C12.4905 16.4852 12.6275 16.4104 12.777 16.3795C12.9265 16.3485 13.0819 16.3628 13.2233 16.4205C13.3647 16.4782 13.4857 16.5767 13.5708 16.7034C13.656 16.8302 13.7015 16.9794 13.7016 17.1321C13.7032 17.3338 13.6249 17.5279 13.4838 17.672C13.3426 17.8161 13.1501 17.8984 12.9484 17.9008Z"></path>
         </svg>
       </span>
        Add to Cart
                    </button>
                  </div>
                  <div class="ya xa">
                    <button
                      class="tb je le we bg mh yg si ui bj jj hk"
                    >
                      Buy Now
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div
        x-show=" modalOpen "
        class="c h m aa sb wb oc je le bg ig"
      >
        <div
          @click.outside=" modalOpen = false "
          class="da rb _c/5 nl/4 ao/2"
        >
          <img
            src="src/assets/ecom-images/products-details/details-02/big-image-01.jpg"
            alt="products-details"
            class="oc ve"
            x-show="productOne "
          />
          <img
            src="src/assets/ecom-images/products-details/details-02/big-image-02.jpg"
            alt="products-details"
            class="oc ve"
            x-show="productTwo "
          />
          <img
            src="src/assets/ecom-images/products-details/details-02/big-image-03.jpg"
            alt="products-details"
            class="oc ve"
            x-show="productThree "
          />
          <img
            src="src/assets/ecom-images/products-details/details-02/big-image-04.jpg"
            alt="products-details"
            class="oc ve"
            x-show="productFour "
          />
        </div>
      </div>
    </section>
    <!-- ====== Products Details End -->

    <!-- ====== Reviews Section Start -->
    <section class="wf ki li">
      <div class="a da">
        <div class="ea sb he">
          <div class="oc vg bq/12">
            <div class="la ff nf bi">
              <div class="kb">
                <div class="je me hm">
                  <div class="xa sb je">
                    <div
                      class="ta gc xc se ye"
                    >
                      <img
                        src="src/assets/ecom-images/reviews/review-05/image-01.png"
                        alt="author"
                        class="wb oc mg ng"
                      />
                    </div>
                    <div class="sb je">
                      <h3
                        class="ya if nf hi wi dj gj"
                      >
                        Wilium Heli
                      </h3>
                      <p class="xi bj hj">
                        22, April 2025
                      </p>
                    </div>
                  </div>

                  <div class="xa sb je">
                    <div class="ya sb je">
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                    </div>
                    <p class="wi dj hj">
                      Rating:
                      <span class="cj gj"> 5.0 </span>
                    </p>
                  </div>
                </div>

                <div class="la">
                  <h4
                    class="xa wi cj gj am"
                  >
                    This is one of the best product.
                  </h4>
                  <p
                    class="qa ui dj hj"
                  >
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Maecenas ac risus a risus elementum vehicula. Class aptent
                    taciti socil ad litora torquent per conubia nostra, per
                    inceptos himenaeos.
                  </p>
                  <button
                    class="tb je le we yf eh _g si ui bj jj hk"
                  >
                    Reply
                  </button>
                </div>
              </div>
              <div class="kb cl em">
                <div class="je me hm">
                  <div class="xa sb je">
                    <div
                      class="ta gc xc se ye"
                    >
                      <img
                        src="src/assets/ecom-images/reviews/review-05/image-02.png"
                        alt="author"
                        class="wb oc mg ng"
                      />
                    </div>
                    <div class="sb je">
                      <h3
                        class="ya if nf hi wi dj gj"
                      >
                        Frank Lerry
                      </h3>
                      <p class="xi bj hj">
                        22, April 2025
                      </p>
                    </div>
                  </div>
                </div>

                <div>
                  <p class="ui dj hj">
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Maecenas ac risus a risus elementum vehicula. Class aptent
                    taciti socil ad litora torquent per conubia nostra, per
                    inceptos himenaeos.
                  </p>
                </div>
              </div>
            </div>

            <div class="la ff nf bi">
              <div>
                <div class="je me hm">
                  <div class="xa sb je">
                    <div
                      class="ta gc xc se ye"
                    >
                      <img
                        src="src/assets/ecom-images/reviews/review-05/image-01.png"
                        alt="author"
                        class="wb oc mg ng"
                      />
                    </div>
                    <div class="sb je">
                      <h3
                        class="ya if nf hi wi dj gj"
                      >
                        Wilium Heli
                      </h3>
                      <p class="xi bj hj">
                        22, April 2025
                      </p>
                    </div>
                  </div>

                  <div class="xa sb je">
                    <div class="ya sb je">
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                    </div>
                    <p class="wi dj hj">
                      Rating:
                      <span class="cj gj"> 5.0 </span>
                    </p>
                  </div>
                </div>

                <div>
                  <h4
                    class="xa wi cj gj am"
                  >
                    This is one of the best product.
                  </h4>
                  <p
                    class="qa ui dj hj"
                  >
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Maecenas ac risus a risus elementum vehicula. Class aptent
                    taciti socil ad litora torquent per conubia nostra, per
                    inceptos himenaeos.
                  </p>
                  <button
                    class="tb je le we yf eh _g si ui bj jj hk"
                  >
                    Reply
                  </button>
                </div>
              </div>
            </div>

            <div class="bi">
              <div>
                <div class="je me hm">
                  <div class="xa sb je">
                    <div
                      class="ta gc xc se ye"
                    >
                      <img
                        src="src/assets/ecom-images/reviews/review-05/image-01.png"
                        alt="author"
                        class="wb oc mg ng"
                      />
                    </div>
                    <div class="sb je">
                      <h3
                        class="ya if nf hi wi dj gj"
                      >
                        Wilium Heli
                      </h3>
                      <p class="xi bj hj">
                        22, April 2025
                      </p>
                    </div>
                  </div>

                  <div class="xa sb je">
                    <div class="ya sb je">
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                      <span class="db">
                        <svg
                          width="20"
                          height="20"
                          viewBox="0 0 20 20"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M18.6562 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75 0.84375 9.5625 1.21875L7 6.625L1.375 7.46875C0.968746 7.53125 0.812496 8.0625 1.125 8.34375L5.21875 12.5625L4.25 18.4688C4.1875 18.875 4.59375 19.2188 4.96875 18.9688L10.0625 16.1875L15.125 18.9688C15.4687 19.1562 15.9062 18.8438 15.8125 18.4688L14.8437 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6562 7.46875Z"
                            fill="#FFA645"
                          />
                        </svg>
                      </span>
                    </div>
                    <p class="wi dj hj">
                      Rating:
                      <span class="cj gj"> 5.0 </span>
                    </p>
                  </div>
                </div>

                <div>
                  <h4
                    class="xa wi cj gj am"
                  >
                    This is one of the best product.
                  </h4>
                  <p
                    class="qa ui dj hj"
                  >
                    Lorem ipsum dolor sit amet, consectetur adipiscing elit.
                    Maecenas ac risus a risus elementum vehicula. Class aptent
                    taciti socil ad litora torquent per conubia nostra, per
                    inceptos himenaeos.
                  </p>
                  <button
                    class="tb je le we yf eh _g si ui bj jj hk"
                  >
                    Reply
                  </button>
                </div>
              </div>
            </div>

            <div class="oh xl">
              <h4
                class="xa vi cj gj bm mp pq"
              >
                Write a Review
              </h4>
              <p class="wa ui dj gj">
                Click on star to review
              </p>
              <div class="ma sb je">
                <span class="db">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M18.6563 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75001 0.84375 9.56251 1.21875L7.00001 6.625L1.37501 7.46875C0.968761 7.53125 0.812511 8.0625 1.12501 8.34375L5.21876 12.5625L4.25001 18.4688C4.18751 18.875 4.59376 19.2188 4.96876 18.9688L10.0625 16.1875L15.125 18.9688C15.4688 19.1562 15.9063 18.8438 15.8125 18.4688L14.8438 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6563 7.46875Z"
                      fill="#FFA645"
                    />
                  </svg>
                </span>
                <span class="db">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M18.6563 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75001 0.84375 9.56251 1.21875L7.00001 6.625L1.37501 7.46875C0.968761 7.53125 0.812511 8.0625 1.12501 8.34375L5.21876 12.5625L4.25001 18.4688C4.18751 18.875 4.59376 19.2188 4.96876 18.9688L10.0625 16.1875L15.125 18.9688C15.4688 19.1562 15.9063 18.8438 15.8125 18.4688L14.8438 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6563 7.46875Z"
                      fill="#FFA645"
                    />
                  </svg>
                </span>
                <span class="db">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M18.6563 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75001 0.84375 9.56251 1.21875L7.00001 6.625L1.37501 7.46875C0.968761 7.53125 0.812511 8.0625 1.12501 8.34375L5.21876 12.5625L4.25001 18.4688C4.18751 18.875 4.59376 19.2188 4.96876 18.9688L10.0625 16.1875L15.125 18.9688C15.4688 19.1562 15.9063 18.8438 15.8125 18.4688L14.8438 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6563 7.46875Z"
                      fill="#FFA645"
                    />
                  </svg>
                </span>
                <span class="db">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M18.6563 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75001 0.84375 9.56251 1.21875L7.00001 6.625L1.37501 7.46875C0.968761 7.53125 0.812511 8.0625 1.12501 8.34375L5.21876 12.5625L4.25001 18.4688C4.18751 18.875 4.59376 19.2188 4.96876 18.9688L10.0625 16.1875L15.125 18.9688C15.4688 19.1562 15.9063 18.8438 15.8125 18.4688L14.8438 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6563 7.46875Z"
                      fill="#FFA645"
                    />
                  </svg>
                </span>
                <span class="db">
                  <svg
                    width="18"
                    height="18"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M18.6563 7.46875L13 6.59375L10.4375 1.21875C10.25 0.84375 9.75001 0.84375 9.56251 1.21875L7.00001 6.625L1.37501 7.46875C0.968761 7.53125 0.812511 8.0625 1.12501 8.34375L5.21876 12.5625L4.25001 18.4688C4.18751 18.875 4.59376 19.2188 4.96876 18.9688L10.0625 16.1875L15.125 18.9688C15.4688 19.1562 15.9063 18.8438 15.8125 18.4688L14.8438 12.5625L18.9375 8.34375C19.1875 8.0625 19.0625 7.53125 18.6563 7.46875Z"
                      fill="#FFA645"
                    />
                  </svg>
                </span>
              </div>

              <form>
                <div class="fa sb he">
                  <div class="oc wg ll/2">
                    <div class="ma">
                      <input
                        type="text"
                        placeholder="First Name"
                        class="oc ve af nf zg yg dj hj rj _j ak lk pk qk rk"
                      />
                    </div>
                  </div>
                  <div class="oc wg ll/2">
                    <div class="ma">
                      <input
                        type="text"
                        placeholder="Last Name"
                        class="oc ve af nf zg yg dj hj rj _j ak lk pk qk rk"
                      />
                    </div>
                  </div>
                  <div class="oc wg">
                    <div class="ma">
                      <input
                        type="text"
                        placeholder="Type review headline"
                        class="oc ve af nf zg yg dj hj rj _j ak lk pk qk rk"
                      />
                    </div>
                  </div>
                  <div class="oc wg">
                    <div class="qa">
                      <textarea
                        rows="6"
                        placeholder="Write your Review"
                        class="oc ve af nf zg yg dj hj rj _j ak lk pk qk rk"
                      ></textarea>
                    </div>
                  </div>
                  <div class="oc wg">
                    <div>
                      <button
                        class="tb je le we yf zg dh si ui bj jj hk"
                      >
                        Submit Review
                      </button>
                    </div>
                  </div>
                </div>
              </form>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!-- ====== Reviews Section End -->

  @endsection 