...
{{--                <select aria-label="select" class="focus:text-indigo-600 focus:outline-none bg-transparent ml-1">--}}
                   @foreach($listCate as $cate)
                        <a class="ml-2" href="{{route('fill-product/'.$cate->id)}}">{{$cate->name}}</a>
                   @endforeach
{{--                </select>--}}
            </div>
        </div>
    ...
    <div class="bg-white py-4 md:py-7 px-4 md:px-8 xl:px-10">
        <div class="sm:flex items-center justify-between">
            <div class="flex items-center">
                <a class="rounded-full focus:outline-none focus:ring-2  focus:bg-indigo-50 focus:ring-indigo-800" href="{{route('list-product')}}">
            ...
            <button onclick="popuphandler(true)" class="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-600 mt-4 sm:mt-0 inline-flex items-start justify-start px-6 py-3 bg-indigo-700 hover:bg-indigo-600 focus:outline-none rounded">
                <p ><a class="text-sm font-medium leading-none text-white" href="{{BASE_URL.'add-product'}}">Add Product</a></p>
            </button>
        </div>
                    ...
                    <tr tabindex="0" class="focus:outline-none h-16 border border-gray-100 rounded">
                        ...
                        <td class="pl-5">
                            <div class="flex items-center w-12 h-12">
                                <img class="object-cover w-full h-full " src="{{BASE_URL.'public/uploads/products/'.$product->image}}" alt="">
                            </div>
                        </td>
                        <td class="pl-5">
                            <div class="flex items-center">
                        ...
                        <td class="pl-5">
                            <div class="flex items-center">
                                ...
                                <p class="text-sm leading-none text-gray-600 ml-2">SL: {{$product->quantity}}</p>
                            </div>
                        </td>


                        <td class="pl-5">
                            <p class="text-sm leading-none text-gray-600 ml-2">{{$product->cate_id}}</p>
                        </td>
                        <td class="pl-5">
                            <div class="flex items-center">
                                ...
                                <p class="text-sm leading-none text-gray-600 ml-2">{{$product->detail}}</p>
                            </div>
                        </td>
                        <td class="px-5  whitespace-nowrap">
                            <div class="box__action flex gap-2">
                                <a href="{{ route('edit-product/'. $product->id) }}  " class="text-indigo-600 hover:text-indigo-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </a>
                                <a href="{{ route('delete-product').'/' . $product->id}}" class="text-indigo-600 hover:text-indigo-900"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
